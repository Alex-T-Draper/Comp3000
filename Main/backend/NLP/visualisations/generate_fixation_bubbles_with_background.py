"""Generate fixation bubble charts with document screenshots as background."""
...
from gaze_utils import (DB_PATH, BUBBLES_BG_DIR, SCREEN_W, SCREEN_H,
                         apply_scroll_adjustment, get_sessions,
                         get_gaze_points_with_time, session_label,
                         get_screenshot_path)
from generate_fixation_bubbles import make_fixation_bubbles_from_pixels


def main():
    BUBBLES_BG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for row in sessions:
        ...
        make_fixation_bubbles_from_pixels(px, py, ts, title, BUBBLES_BG_DIR / filename, bg_image=bg)

    # Combined per condition
    print("\n--- Combined fixation bubbles (with background) by condition ---")
    ...
    for (condition,) in conditions:
        ...
        make_fixation_bubbles_from_pixels(px, py, data[:, 2], title, BUBBLES_BG_DIR / filename, bg_image=bg)

    print(f"\nAll fixation bubbles (with background) saved to: {BUBBLES_BG_DIR.resolve()}")
